...
2. **Add an AST node** — a new `Ore::Foo_Expr < Expression` in [`expressions.rb`](src/compiler/expressions.rb). Only add `attr_accessor`s for what's structurally new; `value`/`type`/`l0..c1`/`source_file` are inherited.
...
5. **Interpret it** — add a case to `Interpreter#interpret`'s dispatch and a new `interp_foo` in [`interpreter.rb`](src/runtime/interpreter.rb) that walks the `Foo_Expr` and produces a runtime value (an `Ore::*` instance, a Ruby primitive, `nil`, etc.).
...
#### Ruby-backed types (`Foo {}` + `Ore::Foo`)

Two files, independently optional — pure-Ore types skip #2, rare Ruby-only types skip #1:

1. **`ore/foo.ore`** — the Ore-level declaration (`Foo { ... }`). A method that defers to Ruby is just `some_method {; @ruby }`.
2. **`src/external/ruby/foo.rb`** — `class Foo < Ore::Instance` (or `< Ore::Type`), inside `module Ore`. `extend Ruby_Proxies` + `proxy :method_name` for 1:1 delegation ([`ruby_proxies.rb`](src/shared/ruby_proxies.rb)), or hand-write `def proxy_method_name(...)` for custom logic. `@ruby` calls `proxy_#{method_name}` on the backing instance.
3. **Register the Ruby file** — `require_relative 'external/ruby/foo'` in [`src/ore.rb`](src/ore.rb)'s "External Ruby-backed built-ins" block (after `runtime/scopes`).
4. **Load the Ore file** — `@load 'ore/foo.ore'` in [`ore/preload.ore`](ore/preload.ore) for always-on, or leave opt-in for the user's own program to `@load` (e.g. `ore/database.ore`).
5. Nothing else — matching Ore type ↔ Ruby class is by name, dynamic at construction time (next section).

#### Linking an instance to its runtime type

1. `Interpreter#find_ruby_class_for_type(type)` walks `type.types` (most-derived first), returns the first Ruby constant `Ore::#{type_name}` that's a `Class < Ore::Instance`.
2. `Interpreter#build_instance_of_type(type, expr)` calls #1: found → `ruby_class.new`; not found → `Ore::Instance.new(type.name)`.
3. Either way: `instance.enclosing_scope = type`, `.structure` bound if any, then the type's own Ore-level body runs on it.
...
#### Instance/Type without a backing `Ore::Class`

1. Perfectly valid — most user `Type { }`s have no Ruby class; `build_instance_of_type` falls back to plain `Ore::Instance.new(type.name)`.
2. A plain `Ore::Instance` works normally for everything declared in Ore — `declarations` hash, methods, `new{;}`, composition, structs.
3. Only `@ruby` breaks:
   - Outside a `Func` scope → `Ore::Invalid_Ruby_Proxy_Directive_Usage`.
   - Instance doesn't `respond_to?("proxy_#{method_name}")` (no Ruby class, or Ruby class missing that one `proxy_*` method) → `Ore::Missing_Ruby_Proxy_Declaration`.
4. So: forgetting the Ruby class is safe unless the `.ore` body calls `@ruby` — then it's a runtime error on first call, not at declaration time.
...
| Standard library (`.ore` side of built-ins) | [`ore/`](ore), auto-loaded via [`ore/preload.ore`](ore/preload.ore) |
| Entry points (`Ore.lex`/`.parse`/`.interp`, `+_file` variants) | [`src/ore.rb`](src/ore.rb) |
