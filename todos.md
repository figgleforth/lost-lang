...
- Interpolation pipes in multiline text aren't escaped properly (`src/compiler/lexer.rb`, repro in `examples/basic_page.ore`) — causes the interpreter to mis-interpolate.
- Dict keys that shadow a built-in dict method name (`keys`, `values`, etc.) break lookup — should check the dict's own scope before falling back to the built-in (`src/runtime/interpreter.rb`, ~L927).
- `String#split` requires an explicit `nil` default arg or it fails (`ore/string.ore`).
- `type_checker_test.rb` — a test that should currently be failing and isn't; smells like a latent type-checker bug, worth an isolated repro before trusting the type checker further.
- Seven separate `# todo: Proper error` placeholders scattered through `src/runtime/interpreter.rb` — real error types needed instead of generic ones.
