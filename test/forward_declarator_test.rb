...
class Forward_Declarator_Test < Base_Test
	...
	def test_literal_rhs_is_preserved_not_dropped
		decls = Ore.declare "greeting := 'hi'"
		assert_instance_of Ore::String_Expr, decls['greeting'].expr_or_decl
	end

	def test_plain_identifier_rhs_is_not_double_wrapped
		decls = Ore.declare "flag := true"
		assert_instance_of Ore::Identifier_Expr, decls['flag'].expr_or_decl
	end

	def test_named_struct_gets_its_declaring_name
		decls = Ore.declare "Named <String, Number>"
		assert_equal 'Named', decls['Named'].expr.structure.name
	end

	def test_conditional_branches_flatten_into_enclosing_scope
		decls = Ore.declare <<~ORE
		    check {;
		    	if true
		    		a := 1
		    	elwhile false
