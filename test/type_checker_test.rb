...
class Type_Checker_Test < Base_Test
	def assert_type_error & block
		assert_raises Lost::Type_Checking_Failed, &block
	end

	def refute_type_error & block
		refute_raises Lost::Type_Checking_Failed, &block
	end

	# --- Happy paths ---

	def test_string_annotation_with_string_literal
		refute_type_error { Lost.type_check "x: String = 'hello'" }
	end

	def test_number_annotation_with_number_literal
		refute_type_error { Lost.type_check 'x: Number = 42' }
	end

	def test_symbol_annotation_with_symbol_literal
		refute_type_error { Lost.type_check 'x: Symbol = :ok' }
	end

	def test_unannotated_assignment_is_not_checked
		refute_type_error { Lost.type_check "x := 123" }
		refute_type_error { Lost.type_check "x := 'hello'" }
		refute_type_error { Lost.type_check "x := :sym" }
	end

	def test_unknown_rhs_is_skipped
		# Identifier on RHS with unknown type, so no error. todo: Maybe print a warning?
		refute_type_error { Lost.type_check "y := 1, x: Number = y" }
	end

	# --- Mismatches at top level ---

	def test_string_annotation_with_number_literal
		assert_type_error { Lost.type_check "x: String = 123" }
	end

	def test_string_annotation_with_symbol_literal
		assert_type_error { Lost.type_check "x: String = :hello" }
	end

	def test_number_annotation_with_string_literal
		assert_type_error { Lost.type_check "x: Number = 'hello'" }
	end
	...
	def test_symbol_annotation_with_string_literal
		assert_type_error { Lost.type_check "x: Symbol = 'hello'" }
	end

	def test_symbol_annotation_with_number_literal
		assert_type_error { Lost.type_check "x: Symbol = 42" }
	end
	...
	def test_mismatch_nested_in_type_body
		assert_type_error { Lost.type_check "Point { x: Number = 'bad' }" }
	end

	def test_valid_annotation_nested_in_type_body
		refute_type_error { Lost.type_check "Point { x: Number = 0 }" }
	end

	# --- Nested in conditional ---

	def test_mismatch_in_if_when_true_branch
		assert_type_error { Lost.type_check "if true \n x: String = 99 \n end" }
	end
	...
	def test_valid_annotation_in_conditional
		refute_type_error { Lost.type_check "if true \n x: String = 'ok' \n end" }
	end

	# --- Nested in for loop ---

	def test_mismatch_in_for_loop_body
		assert_type_error { Lost.type_check "for [1, 2, 3] \n x: String = 99 \n end" }
	end

	def test_valid_annotation_in_for_loop_body
		refute_type_error { Lost.type_check "for [1, 2, 3] \n x: Number = 1 \n end" }
	end

	# --- Nested in prefix expression ---

	def test_mismatch_inside_prefix_expression
		assert_type_error { Lost.type_check "!(x: Number = 'bad')" }
	end
	...
	def test_bug_that_needs_fixing
		...
		refute_type_error { Lost.type_check "name: String = nil\nname = 1234" }
	end
end
