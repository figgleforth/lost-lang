...
class Enums_Test < Base_Test
	def test_empty_enum
		out = Ore.parse <<~CODE
		    My_Enum :: {}
		CODE
		assert_kind_of Ore::Enum_Expr, out.first
	...
	def test_bare_member_gets_its_own_unique_value
		...
		assert_kind_of Ore::Nil_Init_Expr, member
	...
	def test_member_with_type_annotation_only
		...
		CODE
		member = out.first.expressions.first
		assert_kind_of Ore::Identifier_Expr, member
	...
	def test_member_with_self_declared_value
		...
		CODE
		member = out.first.expressions.first
		assert_kind_of Ore::Infix_Expr, member
		assert_equal ':=', member.operator.value
		assert_equal 'ABC', member.left.value
		assert_kind_of Ore::Number_Expr, member.right
	...
	def test_member_with_type_annotation_and_value
		...
		assert_kind_of Ore::Infix_Expr, member
		assert_equal '=', member.operator.value
		assert_kind_of Ore::Identifier_Expr, member.left
		assert_equal 'ABC', member.left.value
	...
	def test_nested_enum_member
		...
		CODE
		member = out.first.expressions.first
		assert_kind_of Ore::Enum_Expr, member
	...
	# `Ore::Enum.new` (no name argument) used to bake "Instance" into @declarations['name'] at construction time; a later `.name =` (a plain Ruby attr write) never touched it, so an enum's own name was permanently wrong at the Ore level.
